 ...
 *2)设备号码直接指定的方式
 *3)ioctl命令码的生成方式
 *在对应的地方，都有说明其不标准的原因。另外，一般都把模块的init和exit函数放在最后。
 *在下一个例子中会给出一个相对典型的方法。
 *二、使用说明：
 *1)编译
 *$make
 *2)插入模块
 *insmod globalmem_simple.ko
 *可以通过"cat /proc/devices"得知设备的主从号码
 *3)创建设备文件
 *#mknod mytest c 1025 0
 *4)使用
 *写入数据：echo hello >mytest
 *读取数据：cat mytest
 *ioctrl：使用testDemo中的代码进行测试。
 *5)卸载模块
...
/*这个宏是一个调试用的开关，如果想要动态分配设备号,那么就设置成1*/
#define DYNAMIC_ALLOC_MAJOR 0
...
int globalmem_init(void)
{
	...
	if(globalmem_major)
	{
		...
		result = register_chrdev_region(devno, 1, "globalmem_simple");
	}
	else
	{
#endif
