...
static int findWindowByTitle (Display *disp, char *title, Window **win)
{
	...
	g_print("go into findWindowByTitle********\n");
#endif
...
#if MY_DEBUG_OUTPUT == 1
	g_print("go outof findWindowByTitle********\n");
#endif
...
/*下面有问号的分支需要改变第2个参数为窗口标题。*/
		...
		else if(strcmp(argv[0],"/usr/bin/StartVideo.sh")== 0)
		{/*videoPath*/
			launchApp_portable("/usr/bin/StartVideo.sh", "VideoPlayer");
		}
		else if(strcmp(argv[0],"/usr/bin/abiword")== 0)
		{/*word*/
			launchApp_portable("/usr/bin/abiword", "abiword.Abiword");
		}
		...
		{/*mail*/
			launchApp_portable("/usr/bin/email", "claws-mail.Claws-mail");
		}
		else if(strcmp(argv[0],"/usr/bin/Tuxpuck")== 0)
		{/*a game,but the window??*/
			launchApp_portable("/usr/bin/Tuxpuck", "TuxPuck");
		}
		else if(strcmp(argv[0],"/usr/bin/madbomber")== 0)
		{/*?????*/
			launchApp_portable("/usr/bin/madbomber", "Mad Bomber");
		}
		...
		{/*/usr/bin/IE*/
			launchApp_portable("/usr/bin/IE", "Navigator.Iceweasel");
		}
		...
		{/*/usr/bin/record*/
			launchApp_portable("/usr/bin/record", "VoiceRecorder");
		}
		...
		else if(strcmp(argv[0],"/usr/bin/TS")== 0)
		{/*/home/Marvell-MID/nfs/usr/bin/TS??????*/
			launchApp_portable("/usr/bin/TS", "videoWinTitle");
		}
		...
		{/*/usr/bin/iceweasel,browser*/
			launchApp_portable("/usr/bin/iceweasel", "Navigator.Iceweasel");
		}
	...
	{
		g_print("command process error!\n");
		return 1;
	}
#if MY_DEBUG_OUTPUT == 1
	g_print("go outof launchApp******\n");
#endif
	return 0;
}
