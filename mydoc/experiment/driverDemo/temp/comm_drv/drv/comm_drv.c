/*
Functions:
(*) Simple log functions controlled by macro.
(*) Support many devices with different minors but the same major.
(*) Judge the current kernel code version, and compare it to specified.
(*) ioctl function be implemented in many ways.
(*) some sample for use of vmalloc and kmalloc.
(*) Message buffer are arranged by kernel list.
(*) Keep device open reference count by atom variable.
(*) Block & Nonblock send/receive support by wait_queue.
*/
...
#if 0
		/*This can be use as a sample of how to use vmalloc.*/
...
#if 0
		if(down_interruptible(&devp[minor].cd_wsem))
		{
			return -ERESTARTSYS;
		}
#endif
		while(devp[minor].cd_tlist_full)
		{
			comm_error("Buffer full!\n");
#if 0
			up(&devp[minor].cd_wsem);
#endif
