...
static int mapmem_fasync(int fd, struct file *filp, int on);
/**
* @}
*/
...
static int g_mapmem_major = __MAPMEM_MAJOR__;

static const struct file_operations g_mapmem_fops = 
{
	.owner = THIS_MODULE,
...
static int mapmem_release(struct inode *inode, struct file *filp)
{
	//struct mapmem_dev *dev = filp->private_data;
	mapmem_fasync(-1, filp, 0);
	return 0;
}
...
static ssize_t mapmem_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	...
	printk(KERN_ALERT "p value is:%d, count value is: %d. \n", p, count);
...
*/
	printk(KERN_ALERT "begin to map!\n");
	unsigned long phyaddr = __pa(g_dev.mem_buf);
	unsigned long size = vma->vm_end - vma->vm_start;
	if(size > __MAPMEM_SIZE__)
	{
		printk(__DEVICE_NAME__"mmap size too big\n");
		return -ENXIO;
	}
	if(remap_pfn_range(vma, vma->vm_start, phyaddr, size, PAGE_SHARED))
	{
		printk(__DEVICE_NAME__"remap_pfn_range failed\n");
		return -ENXIO;
	}
	printk(KERN_ALERT "end of map!\n");
	return 0;
}

static int mapmem_ioctl(struct inode *inodep, struct file *filp, unsigned int cmd, void __user *arg)
{
	printk(KERN_ALERT "begin ioctl!\n");
	int ret = 0;
	struct mapmem_dev *dev = filp->private_data;
	switch(cmd)
	{
		...
		break;
#if 0 
		case __MEM_SEND_NOTIFY__:
		printk(KERN_ALERT "ioctl __MEM_SEND_NOTIFY__!\n");
		if(dev->async_queue)
		{
		...
		break;
#endif 
		case __MEM_RECEIVE_NOTIFY__:
		printk(KERN_ALERT "ioctl __MEM_READ_NOTIFY__!\n");
		if(dev->async_queue)
		{
			;
...
{
	int result;
	dev_t devno = MKDEV(g_mapmem_major,0);
	...
	//g_dev.mem_buf = vmalloc(__MAPMEM_SIZE__);
	if(!g_dev.mem_buf)
	{
      printk(KERN_ERR"%s,%d, get_free_pages failed for g_dev.mem_buf. \n",__func__, __LINE__); 
      return -ENOMEM; 
	}
	memset(g_dev.mem_buf, '%', __MAPMEM_SIZE__);

	cdev_init(&g_dev.cdev, &g_mapmem_fops);
...
void mapmem_exit(void)
{
	...
	free_pages(g_dev.mem_buf,get_order(__MAPMEM_SIZE__));
	//vfree(g_dev.mem_buf);
	unregister_chrdev_region(MKDEV(g_mapmem_major, 0), 1);
}
