...
def get_keys(whole_page):
    sub_pattern = r'^.*neusoft_attendance_online'
    sub_pattern += r'.*?>.*?<.*?name\s*=\s*"(?P<unknown_key>\S*)"'
    sub_pattern += r'.*?title\s*=.*?<\s*input.*?name\s*=\s*"(?P<user_key>\S*)"'
    sub_pattern += r'.*?>.*?<\s*input.*?name\s*=\s*"(?P<passwd_key>\S*)".*$'
    pattern = re.compile(sub_pattern,re.S)
    match = pattern.match(whole_page)
    if None != match:
        unknown_key = match.group("unknown_key")
        user_key = match.group("user_key")
        passwd_key = match.group("passwd_key")
